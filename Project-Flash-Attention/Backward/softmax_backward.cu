...
__device__ inline void warpReduce(volatile float* temp_sums) {
    temp_sums[threadIdx.x] += temp_sums[threadIdx.x + 32];
    temp_sums[threadIdx.x] += temp_sums[threadIdx.x + 16];
    temp_sums[threadIdx.x] += temp_sums[threadIdx.x + 8];
    temp_sums[threadIdx.x] += temp_sums[threadIdx.x + 4];
    temp_sums[threadIdx.x] += temp_sums[threadIdx.x + 2];
    temp_sums[threadIdx.x] += temp_sums[threadIdx.x + 1];
    __syncthreads();
}
...
__global__ void fused_softmax2(const float *P_, const float* dP_, float* dS_, int N, int batch_size, int num_heads) {
    ...
    for(int i = 0; i < N; i++) {
        ...
        shPdP[threadIdx.x] = shP[threadIdx.x] * dP[i * N + threadIdx.x];
        __syncthreads();
        temp_sums[threadIdx.x] = shPdP[threadIdx.x] + shPdP[threadIdx.x + blockDim.x/2];
        __syncthreads();

        // reduce
        if(blockDim.x >= 512){ temp_sums[threadIdx.x] += temp_sums[threadIdx.x + 256]; __syncthreads(); }
        if(blockDim.x >= 256){ temp_sums[threadIdx.x] += temp_sums[threadIdx.x + 128]; __syncthreads(); }
        if(blockDim.x >= 128){ temp_sums[threadIdx.x] += temp_sums[threadIdx.x + 64]; __syncthreads(); }
        warpReduce(temp_sums);
